...
**`odometer_raw`** (taken on trust from elsewhere) - Cumulative distance, incremented roughly once per 100 m according to blackTeaDisp. cap0007 ends at 14, and 1.4 km is far more than a 47 s parking-lot crawl covers, so this is a running total from earlier riding rather than a per-Capture counter. The 100 m step is unverified: settling it needs a ride of known length.
