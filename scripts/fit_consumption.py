...
def render_header(f):
    ...
    w(" *")
    w(" * WHAT THE FORM LEAVES OUT, and it is the big one. GRADIENT IS NOT")
    w(" * MODELLED AND IS THE DOMINANT RESIDUAL. Speed is not the largest")
    w(" * input to what a kilometre costs - the road is - and without route")
    w(" * knowledge that input is simply missing. Any R^2 below is a")
    w(" * statement about the spans that were fitted, never a claim about")
    w(" * the next ride. Anything that shows a rider a number derived from")
    w(" * these constants is showing them a flat-road estimate, and should")
    w(" * say so rather than implying a precision the model does not have.")
    w(" *")
    w(" * Spans where the Pack was recharged rather than drawn from are kept")
    w(" * in the fit, not dropped. Dropping the downhills while the uphills")
    w(" * stay would bias every coefficient upward, and a biased fit that")
    w(" * looks clean is worse than an honest one with a stated residual.")
    w(" *")
    ...
    w(" * that reads these, and ADR-0005 for why it is built this way.")
    w(" *")
    if f.fitted:
        ...
        w(" *   Captures that contributed: %s." % ", ".join(f.contributing))
        if f.negative:
            w(" *   %d of those span%s recovered energy rather than spending"
              % (f.negative, "" if f.negative == 1 else "s"))
            w(" *   it, and were kept - see the note on gradient above.")
    else:
